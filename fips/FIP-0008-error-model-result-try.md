...
18. Zero-arg helper functions may return `Result<u8,u8>` in stage0, and helper call expressions participate in `try`, `try <expr>`, and postfix `?` using the same deterministic bootstrap constraints as local result bindings.
