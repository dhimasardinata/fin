...
4. `ci/verify_seed_hash.ps1` validates seed metadata, manifest/SHA256SUMS consistency, artifact path safety, hash syntax, configured artifact hashes, and supports `-RequireSet` for release-time enforcement.
...
2. `tests/reproducibility/verify_seed_hash_policy_gate.ps1` validates manifest/SHA256SUMS mismatch rejection, release-required hash enforcement, path safety, hash syntax, artifact format, real artifact hashing, and UNSET/artifact conflict rejection.
