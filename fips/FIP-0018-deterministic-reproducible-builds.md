...
5. Deterministic stage0 Windows target build through `finobj+finld` (`fin build --target x86_64-windows-pe --pipeline finobj`) for fixed inputs.
6. Deterministic package artifact generation (`fin pkg publish`) for fixed inputs.
7. Deterministic stage0 `finobj` writing (`write_finobj_exit`) for fixed inputs.
8. Deterministic stage0 `finld` linking (`link_finobj_to_elf`) for Linux and Windows targets on fixed inputs.
9. Hash-based verification across repeated invocations.
...
1. `tests/reproducibility/verify_stage0_reproducibility.ps1` validates repeated-hash stability for emit/build (direct, Linux+Windows `finobj` pipeline, Windows target)/publish/finobj/finld paths.
