...
19. Stage0 zero-arg helper calls `<ident>()` infer to the callee boundary type: omitted helper return boundaries default to `u8`, explicit helper return boundaries currently allow `u8` or `Result<u8,u8>`, and helper call expressions propagate that inferred type into surrounding arithmetic, assignment, and unwrap checks.
20. Helper functions with omitted return boundaries reject non-`u8` terminal expressions deterministically, keeping stage0 boundary inference explicit without requiring extra type syntax on the common `u8` path.
