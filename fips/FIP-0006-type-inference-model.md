...
22. Stage0 nested block statements create lexical scopes for inferred locals: bindings declared inside a block remain visible only inside that block, outer bindings remain available within nested blocks, and inner block bindings may shadow an outer visible name while still introducing a distinct inferred binding identity.
