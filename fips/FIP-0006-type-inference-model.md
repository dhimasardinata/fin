...
22. Stage0 nested block statements create lexical scopes for inferred locals: bindings declared inside a block remain visible only inside that block, outer bindings remain available within nested blocks, and block-local names must not duplicate a currently visible binding name in bootstrap mode.
23. Terminal `return`/`exit` statements inside nested blocks are still checked against the enclosing function boundary type, and a block-terminal expression propagates that inferred boundary back through the enclosing function.
