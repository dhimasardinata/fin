...
17. Stage0 borrow/dereference inference supports `&<ident>` and `*<expr>`: borrow infers reference type (`&u8` or `&Result<u8,u8>`), dereference expects reference operands and infers underlying value type (`u8` or `Result<u8,u8>`), and dereference rejects consumed reference targets deterministically when source lifecycle is not `alive`.
