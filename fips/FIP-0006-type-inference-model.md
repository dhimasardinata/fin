...
- implementation:
  ...
  - tests/conformance/fixtures/main_exit_minus_equals_literal.fn
  - tests/conformance/fixtures/main_exit_times_equals_literal.fn
  - tests/conformance/fixtures/main_exit_div_equals_literal.fn
  - tests/conformance/fixtures/main_exit_mod_equals_literal.fn
  - tests/conformance/fixtures/main_exit_minus_equals_after_rhs_releases_borrow.fn
  ...
  - tests/conformance/fixtures/main_exit_block_local.fn
  - tests/conformance/fixtures/main_exit_helper_block_return.fn
  ...
  - tests/conformance/fixtures/invalid_div_equals_missing_expression.fn
  - tests/conformance/fixtures/invalid_minus_equals_non_u8_expression.fn
  - tests/conformance/fixtures/invalid_times_equals_non_u8_target.fn
  - tests/conformance/fixtures/invalid_minus_equals_underflow.fn
  - tests/conformance/fixtures/invalid_times_equals_overflow.fn
  - tests/conformance/fixtures/invalid_div_equals_by_zero.fn
  - tests/conformance/fixtures/invalid_mod_equals_by_zero.fn
  - tests/conformance/fixtures/invalid_minus_equals_after_rhs_still_borrowed.fn
...
15. Stage0 mutable compound assignment `<ident> (+=|-=|*=|/=|%=) <expr>` requires an alive mutable `u8` target, infers the RHS through normal expression typing, reuses post-RHS assignment hazard checks, and rejects non-`u8` targets, non-`u8` RHS values, overflow/underflow, division/modulo by zero, and active-borrow conflicts deterministically.
...
17. Stage0 statement-form conditionals `if (<expr>) { ... } [else { ... }]` enforce `u8` condition typing, require block branches, and type-check both branches with copied parser state so diagnostics stay deterministic even when only one branch mutates live execution state.
18. Logical short-circuit still preserves deterministic operand typing rules in inference: non-selected RHS branches do not mutate runtime lifecycle state but are type-checked using copied state to preserve deterministic diagnostics.
19. Stage0 borrow/dereference inference supports `&<ident>` and `*<expr>`: borrow infers reference type (`&u8` or `&Result<u8,u8>`), dereference expects reference operands and infers underlying value type (`u8` or `Result<u8,u8>`), dereference rejects consumed reference targets deterministically when source lifecycle is not `alive`, and non-reference target assignment/move/drop is rejected while active borrows exist (for assignment, borrow conflicts are checked after RHS evaluation so RHS-owned borrow release can succeed).
20. Stage0 helper parameter boundaries require explicit type annotations; current helper parameter types are restricted to `u8` and `Result<u8,u8>`, `main` remains zero-argument, and callee parameter bindings enter scope as immutable alive locals with those declared boundary types.
21. Stage0 helper calls `<ident>([<expr>, ...])` infer to the callee boundary type: omitted helper return boundaries default to `u8`, explicit helper return boundaries currently allow `u8` or `Result<u8,u8>`, helper argument expressions are type-checked left-to-right against parameter boundaries, and helper call expressions propagate the inferred return type into surrounding arithmetic, assignment, and unwrap checks.
22. Helper functions with omitted return boundaries reject non-`u8` terminal expressions deterministically, keeping stage0 boundary inference explicit without requiring extra type syntax on the common `u8` path.
23. Stage0 nested block statements create lexical scopes for inferred locals: bindings declared inside a block remain visible only inside that block, outer bindings remain available within nested blocks, and inner block bindings may shadow an outer visible name while still introducing a distinct inferred binding identity.
24. Terminal `return`/`exit` statements inside nested blocks or a selected statement-form conditional branch are still checked against the enclosing function boundary type, and the selected terminal branch propagates that inferred boundary back through the enclosing function.
