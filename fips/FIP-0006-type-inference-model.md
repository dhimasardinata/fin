...
10. Stage0 arithmetic, comparison, bitwise, and logical operator typing is enforced in inference paths: `+`/`-`/`*`/`/`/`%`/`==`/`!=`/`<`/`<=`/`>`/`>=`/`&`/`^`/`|`/`!`/`&&`/`||` require inferred `u8` operands and reject non-`u8` inferred types deterministically; boolean literals `true`/`false` infer to `u8` values `1`/`0`; comparison/logical expressions infer normalized `u8` predicate values (`0`/`1`); bitwise expressions infer direct `u8` value results with precedence `| < ^ < &` and with bitwise tiers placed below comparison and above logical operators.
