...
4. `drop(<ident>)`
5. `exit(<expr>)` and `return <expr>` / `return(<expr>)` (terminal statement forms)
...
5. `try(<expr>)` and `<expr>?` (stage0 bootstrap unwrap forms)
