...
- implementation:
  ...
  - tests/conformance/fixtures/invalid_block_duplicate_visible_name.fn
...
9. `drop(<ident>)`
10. `exit(<expr>)` and `return <expr>` / `return(<expr>)` (terminal statement forms)
...
2. `tests/conformance/verify_stage0_grammar.ps1` validates stage0 `u8` arithmetic (`+`, `-`, `*`, `/`, `%`), comparisons (`==`, `!=`, `<`, `<=`, `>`, `>=`), shifts (`<<`, `>>`), bitwise operators (`&`, `^`, `|`), unary/binary logical operators (`!`, `~`, `&&`, `||`), bootstrap unwrap forms (`try(<expr>)`, `try <expr>`, and postfix `<expr>?`), unwrap-binding sugars (`let <ident> ?= <expr>` and `var <ident> ?= <expr>`), unwrap-assignment sugar (`<ident> ?= <expr>`), compound assignment (`<ident> += <expr>`), decimal/hexadecimal/binary literals, boolean literals (`true`, `false`), helper declarations/calls with typed value parameters, lexical block statements, terminal `exit`/`return` forms, parenthesized grouping, and `if(cond, then, else)` with deterministic diagnostics for non-`u8` operands, overflow/underflow, division/modulo-by-zero, shift-count range violations, left-shift overflow, missing binary-operator operands, missing unary `!`/`~` operands, malformed/out-of-range hex/binary literals, missing return expressions, statements after terminal exit/return (including block-propagated terminals), empty parenthesized expressions, invalid/missing `if` arguments, non-`u8` conditions, reserved-keyword identifier misuse (`true`/`false`), branch type mismatches, invalid bootstrap unwrap forms, invalid unwrap-binding/unwrap-assignment/compound-assignment forms, invalid helper call resolution/argument counts/argument types, invalid helper parameter declarations/types, invalid block-visible-name reuse, invalid block-local reference escape, non-zero-argument `main`, duplicate functions, recursive helper graphs, helper-only `exit(...)`, and selected-operand ownership misuse in logical expressions.
