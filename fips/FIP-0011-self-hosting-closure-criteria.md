...
8. Statically validate the committed baseline contract before the heavier closure run, including key order, duplicate/unexpected key rejection, hash formats, parity booleans, direct/finobj equality, source existence, and closure hash derivation from matrix hashes.
9. Materialize closure build outputs in a run-scoped workspace under `artifacts/closure` using run-tokenized output basenames to avoid cross-run collisions in both final outputs and stage0 intermediate finobj temp paths.
