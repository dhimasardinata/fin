...
20. Assignment/move/drop operations on non-reference targets are rejected while active reference bindings still borrow the target; for assignment, conflicts are checked after RHS evaluation so RHS-side reference release can allow assignment, while remaining active borrows still reject deterministically.
