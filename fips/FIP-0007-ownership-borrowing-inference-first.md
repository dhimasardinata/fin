...
29. Nested wrapper paths also preserve type guards for moved `try` inputs; non-result moved identifiers in `ok(try(move(<ident>)))` and `err(try(move(<ident>)))` are rejected deterministically with the same `try(...) expects Result<u8,u8>` diagnostics.
