...
35. Unary logical-not composition (`!<expr>`) preserves ownership transitions of its evaluated operand; for example `!move(<ident>)` consumes the binding and subsequent use is rejected deterministically.
36. Nested block statements create lexical sub-scopes for ownership tracking; block-local bindings are removed when the block exits, block-local reference aliases release outer-target borrows at block exit, and references that would leave the block while still borrowing block-local storage are rejected deterministically.
37. Stage0 lexical shadowing is binding-identity aware: an inner block may reuse an outer visible name, but existing borrows remain attached to the original lexical binding and escape checks operate on binding identity rather than raw identifier text.
...
3. Compound-assignment borrow timing now covers the generalized operator family through `main_exit_minus_equals_after_rhs_releases_borrow.fn` and `invalid_minus_equals_after_rhs_still_borrowed.fn` in addition to the existing `+=` fixtures.
