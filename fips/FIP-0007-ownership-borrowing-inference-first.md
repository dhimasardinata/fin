...
4. Identifier use after `drop(<ident>)` or `move(<ident>)` is rejected deterministically.
5. Double-drop and double-move are rejected deterministically.
6. Assignment-to-dropped-binding and self-move assignment hazards are rejected deterministically.
7. Drop/move on undefined identifiers are rejected deterministically.
8. Expression parser rejects borrow/reference syntax (`&expr`) with deterministic diagnostics.
9. Expression parser rejects dereference syntax (`*expr`) with deterministic diagnostics.
10. Type-annotation parser rejects ownership/borrowing-prefixed type annotations in stage0 bootstrap.
11. This slice creates explicit parser/test safety gates while ownership inference and borrow-check semantics are still evolving.
...
1. `tests/conformance/verify_stage0_grammar.ps1` validates `main_drop_unused.fn` and `main_move_binding.fn` and asserts parse failures for use-after-drop/move, double-drop/move, assign-after-drop, undefined-drop/move, self-move assignment, borrow-reference, dereference, and borrow-type fixtures.
