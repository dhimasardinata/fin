...
27. Transition guards remain enforced for nested source-binding paths; for example `drop(<ident>)` after nested `ok(try(move(<ident>)))` consumption is rejected as drop-after-move until mutable re-initialization restores `alive`.
28. This slice creates explicit parser/test safety gates while ownership inference and borrow-check semantics are still evolving.
