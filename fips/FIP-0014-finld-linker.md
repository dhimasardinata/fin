...
4. `tests/reproducibility/verify_test_tmp_workspace_policy.ps1` validates shared temp-workspace helper behavior (PID-scoped setup/finalize, keep-mode retention, age-gated stale pruning with active-PID + owner-metadata protection, and env validation for stale-hours config).
