...
- `tests/reproducibility/verify_seed_hash_policy_gate.ps1`: seed metadata policy self-check coverage for FIP-0003 manifest/SHA256SUMS consistency, `UNSET` handling, `-RequireSet`, and concrete artifact hash parity.
...
- `verify_finobj_roundtrip.ps1`, `verify_finobj_link.ps1`, `verify_build_pipeline_finobj.ps1`, `verify_build_target_windows.ps1`, `verify_manifest_target_resolution.ps1`, `verify_init.ps1`, `verify_fmt.ps1`, `verify_doc.ps1`, `verify_pkg.ps1`, `verify_pkg_publish.ps1`, `verify_seed_hash_policy_gate.ps1`, `verify_manifest_policy_gate.ps1`, `verify_toolchain_policy_gate.ps1`, and `verify_stage0_reproducibility.ps1` use PID-scoped temp roots under `artifacts/tmp` and prune stale temp dirs from prior runs.
