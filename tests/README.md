...
- `tests/reproducibility/verify_closure_workspace_policy.ps1`: closure run-workspace policy gate (invalid stale-hours fail-fast, owner-metadata active-owner checks, invalid-metadata fallback+repair, legacy PID-only backfill, and mismatched/inactive stale-dir pruning).
