...
- `tests/reproducibility/verify_test_tmp_workspace_policy.ps1`: shared test temp-workspace policy gate (PID-scoped init/finalize, keep-mode retention, age-gated stale pruning with active-PID protection, env validation).
...
- Stale pruning keeps recent temp dirs and skips stale dirs whose PID owner is still active; default staleness window is 6 hours and can be tuned via `FIN_TEST_TMP_STALE_HOURS`.
