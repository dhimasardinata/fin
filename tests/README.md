...
- `tests/reproducibility/verify_ownership_borrow_contract.ps1`: FIP-0007 stage0 ownership/borrow contract gate covering parser lifecycle hooks, conformance diagnostics, runtime fixture coverage, and implemented-status linkage.
...
- `verify_finobj_roundtrip.ps1`, `verify_finobj_link.ps1`, `verify_build_pipeline_finobj.ps1`, `verify_build_target_windows.ps1`, `verify_manifest_target_resolution.ps1`, `verify_init.ps1`, `verify_fmt.ps1`, `verify_doc.ps1`, `verify_pkg.ps1`, `verify_pkg_publish.ps1`, `verify_manifest_policy_gate.ps1`, `verify_toolchain_policy_gate.ps1`, and `verify_stage0_reproducibility.ps1` use PID-scoped temp roots under `artifacts/tmp` and prune stale temp dirs from prior runs. `verify_ownership_borrow_contract.ps1` is a static repository contract check and does not create temp workspaces.
