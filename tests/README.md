...
- `ci/verify_fip_metadata.ps1`: FIP lifecycle metadata and index consistency gate, also checking discussion metadata, `requires` references, status labels, and CI/doctor wiring.
...
- `tests/reproducibility/verify_fip_metadata_policy_gate.ps1`: FIP metadata policy self-check coverage for missing author metadata, malformed/invalid `created` dates, invalid `target_release` values, invalid discussion metadata, index mismatches, missing status labels, missing CI/doctor metadata-verifier wiring, empty acceptance criteria, missing/unsafe implementation paths, invalid/unknown/duplicate/self `requires` entries, status-sensitive empty implementation lists, and implemented/released FIPs that still carry discussion or compatibility placeholders.
