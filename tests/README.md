...
- `tests/reproducibility/verify_closure_workspace_policy.ps1`: closure run-workspace policy gate (invalid stale-hours fail-fast, `FIN_KEEP_CLOSURE_RUNS=1` keep-mode prune bypass, owner-metadata active-owner checks, invalid-metadata fallback+repair, legacy PID-only backfill, and mismatched/inactive stale-dir pruning).
