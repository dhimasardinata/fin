...
- `tests/integration/verify_finobj_link.ps1`: stage0 finld minimal multi-object link gate for Linux ELF and Windows PE, including deterministic order-independence, duplicate rejection, symbol/relocation validation checks, entry relocation materialization checks (`abs32`/`rel32`), relocation-bounds rejection, and `-AsRecord` witness-hash stability (object-set, symbol-resolution, relocation-resolution).
