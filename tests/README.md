...
- `ci/verify_fip_metadata.ps1`: FIP lifecycle metadata and index consistency gate, also checking case-sensitive discussion/status metadata, `requires` references, status labels, and CI/doctor wiring.
...
- `tests/reproducibility/verify_closure_baseline_contract.ps1`: committed closure baseline contract gate covering case-sensitive key order, hash formats, parity booleans, source existence, and derived closure hash consistency.
- `tests/reproducibility/verify_closure_baseline_contract_policy_gate.ps1`: closure baseline contract self-check coverage for wrong-case keys, wrong-case mode values, lowercase `UNSET`, uppercase hashes, and uppercase parity booleans.
...
- `tests/conformance/verify_finobj_roundtrip.ps1`: stage0 finobj reader/writer and malformed-object validation gate, including case-sensitive key validation, canonical symbol-order validation, symbol-value metadata validation (`symbol_values`), and relocation metadata validation (kind defaults + supported-kind checks), with PID-scoped temp workspace hygiene.
- `tests/integration/verify_finobj_link.ps1`: stage0 finld minimal multi-object link gate for Linux ELF and Windows PE, including deterministic order-independence, duplicate rejection, case-sensitive symbol resolution, symbol/relocation validation checks, non-entry relocation rejection, entry relocation materialization checks (Linux `abs32`/`rel32`, Windows `abs32`), symbol-value override behavior, relocation-bounds/invalid-site rejection, Windows unsupported-kind rejection (`rel32`), relocation-patched verifier mode checks (`-AllowPatchedCode`), and `-AsRecord` diagnostics checks (object-set/symbol-resolution/relocation-resolution witness hashes + verify mode fields).
...
- `tests/integration/verify_manifest_target_resolution.ps1`: stage0 target resolution from manifest primary gate with command-path manifest policy enforcement, PID-scoped temp workspace hygiene, direct/finobj parity checks, and stage0 finobj temp artifact cleanup checks.
...
- `tests/integration/verify_pkg.ps1`: `fin pkg add` integration gate with case-sensitive dependency identity, manifest-policy rejection before mutation, and PID-scoped temp workspace hygiene.
- `tests/integration/verify_pkg_publish.ps1`: `fin pkg publish` integration gate with case-sensitive payload path identity, manifest-policy rejection, and PID-scoped temp workspace hygiene.
...
- `tests/reproducibility/verify_manifest_policy_gate.ps1`: manifest policy gate self-check coverage for workspace identity, version, canonical seed-hash syntax, case-sensitive section/key and target schema, dependency entry syntax, canonical booleans, and policy switches, with PID-scoped temp workspace hygiene.
- `tests/reproducibility/verify_seed_hash_policy_gate.ps1`: seed hash policy self-check coverage for manifest/SHA256SUMS consistency, exact artifact path matching, release-required hash enforcement, safe artifact paths, lowercase hash syntax, supported artifact format, seed identity/version and policy booleans, real artifact hash verification, and UNSET/artifact conflict rejection.
- `tests/reproducibility/verify_fip_metadata_policy_gate.ps1`: FIP metadata policy self-check coverage for missing author metadata, malformed/invalid `created` dates, invalid or wrongly cased `target_release`/status/discussion metadata, index mismatches, missing or wrongly cased status labels, missing or wrongly cased CI metadata-verifier wiring, empty acceptance criteria, missing/unsafe implementation paths, invalid/unknown/duplicate/self `requires` entries, status-sensitive empty implementation lists, and implemented/released FIPs that still carry discussion or compatibility placeholders.
- `tests/reproducibility/verify_fip_policy_gate.ps1`: FIP PR-link policy self-check coverage for missing links, CI policy/workflow changes, ineligible review, unknown, eligible FIP statuses, and wrong-case eligible status rejection.
- `tests/reproducibility/verify_stdlib_contract.ps1`: no-libc stdlib contract self-check coverage for `FIP-0017` API lanes, case-sensitive ABI witnesses, and runtime table links.
- `tests/reproducibility/verify_stdlib_contract_policy_gate.ps1`: stdlib contract policy self-check coverage for wrong-case ABI witness paths, FIP implementation paths, runtime ABI witnesses, and suite wiring.
- `tests/reproducibility/verify_toolchain_policy_gate.ps1`: toolchain policy gate self-check coverage for root handling, disallowed workflow commands, case-insensitive matching, and allow-tagged exceptions, with PID-scoped temp workspace hygiene.

Temporary workspace policy:

- `verify_finobj_roundtrip.ps1`, `verify_finobj_link.ps1`, `verify_build_pipeline_finobj.ps1`, `verify_build_target_windows.ps1`, `verify_manifest_target_resolution.ps1`, `verify_init.ps1`, `verify_fmt.ps1`, `verify_doc.ps1`, `verify_pkg.ps1`, `verify_pkg_publish.ps1`, `verify_examples.ps1`, `verify_closure_baseline_contract_policy_gate.ps1`, `verify_manifest_policy_gate.ps1`, `verify_seed_hash_policy_gate.ps1`, `verify_fip_metadata_policy_gate.ps1`, `verify_fip_policy_gate.ps1`, `verify_stdlib_contract_policy_gate.ps1`, `verify_toolchain_policy_gate.ps1`, and `verify_stage0_reproducibility.ps1` use PID-scoped temp roots under `artifacts/tmp` and prune stale temp dirs from prior runs.
