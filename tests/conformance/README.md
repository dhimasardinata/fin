...
- `fixtures/invalid_unwrap_assignment_after_rhs_still_borrowed.fn`: invalid source, parser must reject unwrap-assignment when RHS releases only some borrows and another active reference remains.
- `fixtures/invalid_plus_equals_missing_expression.fn`: invalid source, parser must reject `+=` without a RHS expression.
- `fixtures/invalid_plus_equals_immutable_target.fn`: invalid source, parser must reject compound assignment on immutable targets.
- `fixtures/invalid_plus_equals_non_u8_target.fn`: invalid source, parser must reject compound assignment when target type is not `u8`.
