...
- `verify_fip_metadata_policy_gate.ps1`: validates FIP metadata gate behavior for implementation-path existence, `requires` references, and status-sensitive empty implementation lists.
- `verify_fip_policy_gate.ps1`: validates FIP PR-link gate behavior for non-feature changes, missing links, unknown FIPs, draft FIPs, and eligible accepted/in-progress/implemented FIPs.
