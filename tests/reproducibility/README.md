...
- `verify_closure_workspace_policy.ps1`: validates stage0 closure run-workspace stale-pruning policy with owner-metadata safety (`pid` + `start_utc`), including `FIN_KEEP_CLOSURE_RUNS=1` keep-mode bypass across consecutive runs, mismatched-metadata pruning, invalid-metadata fallback+repair, legacy PID-only backfill, and stale-hours env validation.
