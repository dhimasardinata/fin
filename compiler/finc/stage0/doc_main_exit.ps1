...
- Current grammar subset: fn main() [-> u8] { let/var/assign/exit } with u8 literals, identifier expressions, stage0 bootstrap `ok/err/try` forms (where `try` requires `Result<u8,u8>`), and optional binding annotations `: u8` or `: Result<u8,u8>`. Ownership/borrowing syntax (`&`, `*`) is intentionally rejected in stage0 bootstrap.
"@
