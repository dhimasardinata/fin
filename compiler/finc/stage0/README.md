...
- `parse_main_exit.ps1`: parses stage0 `.fn` subset with typed top-level helper parameters, helper calls, bindings, ownership forms, mutation, statement `if`/`while`, and terminal `exit`/`return`, then resolves final entrypoint exit code.
...
Emitters write final executable images directly (no assembler/linker).
They are intentionally tiny and deterministic to validate the direct-image pipeline.
